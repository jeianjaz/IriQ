...
CREATE POLICY "Users can update own profile" 
  ...
  USING (auth.uid() = id)
  WITH CHECK (auth.uid() = id AND (NEW.role IS NULL OR NEW.role = OLD.role));
...
CREATE POLICY "Anyone can insert device status" 
...
CREATE POLICY "Anyone can update executed status" 
